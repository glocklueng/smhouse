/*********************************************************************
	Rhapsody in C	: 7.6.1 
	Login		: 1
	Component	: ModelExecutionUc1 
	Configuration 	: Animate
	Model Element	: Uc2_InterfacesPkg
//!	Generated Date	: Thu, 10, May 2012  
	File Path	: ModelExecutionUc1\Animate\Uc2_InterfacesPkg.c
*********************************************************************/

/*## auto_generated */
#include "Uc2_InterfacesPkg.h"
/*## auto_generated */
#include "iDriver_Uc_Uc2DataLoggingAndIntrusionDetection.h"
/*## auto_generated */
#include "iScanner_Uc_Uc2DataLoggingAndIntrusionDetection.h"
/*## auto_generated */
#include "iSensors_Uc_Uc2DataLoggingAndIntrusionDetection.h"
/*## auto_generated */
#include "iUc_Uc2DataLoggingAndIntrusionDetection_Scanner.h"
/*## auto_generated */
#include "iUc_Uc2DataLoggingAndIntrusionDetection_Sensors.h"
/*## package InterfacesPkg::Uc2_InterfacesPkg */


#ifdef _OMINSTRUMENT
/*#[ ignore */
static const ARCSerGVtbl InterfacesPkg_Uc2_InterfacesPkg_instrumentVtbl = {
    NULL,
    NULL
};
/*#]*/

OM_INSTRUMENT_PACKAGE(InterfacesPkg_Uc2_InterfacesPkg, InterfacesPkg::Uc2_InterfacesPkg, &InterfacesPkg_Uc2_InterfacesPkg_instrumentVtbl)
#endif /* _OMINSTRUMENT */

/*********************************************************************
	File Path	: ModelExecutionUc1\Animate\Uc2_InterfacesPkg.c
*********************************************************************/
